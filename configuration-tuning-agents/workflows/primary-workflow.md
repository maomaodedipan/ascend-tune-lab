...
| 路径 A · 服务化调优 |   | 路径 B · Profiling 分析 |   | 路径 C · 最佳PD配比 |

| serving-tuning-   |   | profiling-analysis-    |   | pd-ratio-         |

| workflow.md       |   | workflow.md            |   | workflow.md       |

| Phase 0 → 1 → 2   |   | 独立分析流水线           |   | Phase 0 → 1→2→3→4 |
...
> 后续若新增路径，在上表与示意图中平级追加一行/一支即可，**不要**把新能力塞进服务化 Phase 内部。
...
2. **路径互斥**：同一请求只走一条；禁止混跑、禁止在路径 A/B/C 中途插入另一条。

3. **意图不清** → 先问用户选 A / B / C，默认不派发 subagent。

4. **路径 B 缺 `profiler_path`** → 只索取路径并停止；**不得**因此进入路径 A 或 C。

5. **路径 C**：用户提到 PD 配比 / 最佳 P:D 等 → 进入 C；**不得**把配比实测塞进路径 A Phase 2。

6. 派发模板统一见 [`references/subagent-prompt-templates.md`](references/subagent-prompt-templates.md)。
...
各路径落盘目录见对应详文（服务化：`{case_dir}/baseline|tuning/`；Profiling：`{workdir}/profiling/`；PD 配比：`{workdir}/pd-ratio/`）。
