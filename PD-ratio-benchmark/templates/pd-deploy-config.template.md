...
请填写下方必填段后保存，再向 Agent 发起「计算最佳 PD 配比」请求。  
参考：`PD-ratio-benchmark/references/pd-user-config-format.md`、`configuration-tuning-skills/pd-config-env-check/references/kv-connector-and-params.md`。

**约束摘要**：

- 部署顺序：**Mooncake master → Prefill → Decode → Proxy**（除非 `skip_mooncake_master: true`）。
- Prefill / Decode 拉起命令：**推荐填写**；为空则检查步骤从 [官方模型教程](https://docs.vllm.ai/projects/vllm-ascend-cn/zh-cn/latest/tutorials/models/) 按模型名回填（优先 PD 分离 + A2/A3 匹配）。
...
> **SLO**：先向用户确认 TTFT/TPOT；未提供则 **TTFT 不设限制**、**TPOT=50ms**。配比实测：**P 只校验 TTFT，D 只校验 TPOT**；P/D 并发可不同。  
